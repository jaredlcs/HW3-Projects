"""
HW3-3: Custom Dodger Environment + PPO Agent
CSCI6353 Homework 3
Author: Jared Soto
Date: November 2025

This script adapts the PPO (Proximal Policy Optimization) loop from HW3-1
to the custom pygame-based Dodger environment implemented in
`dodger_environment.py`.
"""
...
from dodger_environment import DodgerEnv
...
class Config:
    """Configuration for the DodgerEnv PPO experiment"""

    # Environment meta
    ENV_NAME = "DodgerEnv"
    TARGET_REWARD = 1200  # Survive ~1200 frames on average
    ...
    # Network architecture (Dodger observation = 4 floats, action = 3 moves)
    STATE_DIM = 4
    HIDDEN_DIM = 128
    ACTION_DIM = 3
    ...
    def __init__(self):
        ...
        print(f"\n{'='*60}")
        print("HW3-3: DodgerEnv with PPO")
        print(f"{'='*60}")
...
def train(config, resume=False):
    """Train PPO agent on the custom Dodger environment"""
    ...
    while episode < config.MAX_EPISODES:
        ...
        if len(scores) >= config.EVAL_WINDOW and avg_score >= config.TARGET_REWARD:
            print(f"\n{'='*60}")
            print(f"[INFO] Target reached! Average reward: {avg_score:.2f}")
            print(f"{'='*60}\n")
    ...
    if best_run_data:
        print(f"\nPlotting best run with avg score: {best_run_data['avg_score']:.2f}")
...
def plot_training_curve(scores, config, run_number=None):
    ...
    if len(scores) >= config.EVAL_WINDOW:
        moving_avg = np.array([
            np.mean(scores[max(0, i-config.EVAL_WINDOW+1):i+1])
            for i in range(len(scores))
        ])
        moving_std = np.array([
            np.std(scores[max(0, i-config.EVAL_WINDOW+1):i+1])
            for i in range(len(scores))
        ])
        plt.plot(
            episodes,
            ...
            color="#F28E2B",
            linewidth=2.5,
            label=f'{config.EVAL_WINDOW}-Episode Moving Avg'
        )
        # Shaded band for ±1 std dev to convey stability
        plt.fill_between(
            episodes,
    ...
    title = 'DodgerEnv Training Curve (PPO)'
    if run_number:
    ...
    print(f"[INFO] Training plot saved to {config.PLOT_PATH}")
    plt.close()
...
def test(config, num_episodes=10, render=True):
    """Test trained model on DodgerEnv"""
    ...
    env = DodgerEnv(render_mode="human" if render else None)
...
def main():
    """Main entry point"""
    parser = argparse.ArgumentParser(description='HW3-3: DodgerEnv with PPO')
